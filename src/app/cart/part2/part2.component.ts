...
export class Part2Component implements OnInit {
  ...
  constructor(
    ...
    this.deposits  = Number(JSON.parse(localStorage.getItem("userLevel"))['value'])/100;
  ...
  getItemsCart(){
    ...
    _self._sendItemsData.currentMessage.subscribe(message => {
      if(message.length === 0) 
      _self.router.navigate(['/cart/pharse1']);
      else{
        ...
        _.forEach(_self._allItemsSelect, function(value) {
          ...
          console.log(_self.totalPriceWithNdt);
        });
        ...
        _self.deposits = Number(String(_self.deposits).split(".")[1].charAt(0)) > 5? Math.ceil(_self.deposits) : Math.floor(_self.deposits);
      }
    })
  }
  ...
  setItemBooking(){
      ...
      this.bookingProduct.DeliveryAddressId = Number(this.checkedAddress[0].deliveryAddressId);
  ...
  public validateFunc(): boolean {
    this.checkedAddress = [];
    this.checkedAddress = _.filter(this.lstAddress, function(o) { return o.isDefault; });
    if(this.checkedAddress.length === 0) {
      this.bookingProduct.validate.errAddress = true;
      this.bookingProduct.validate.messAddress = "bạn chưa chọn địa chỉ nhận hàng";
      return false;
    }
  ...
  funcDeposit() {
    ...
    if(checkingValidate) {
      ...
      this.setItemBooking();
      this._cartServices.checkDepositListOrder(this.bookingProduct).subscribe(res => {
  ...
  getWalletInfo(walletId: number) {
    ...
    _self.walletService.getInfoWallet(walletId).subscribe(res => {
      if(res.result.success) {
        ...
        _self.isBill = Number(_self.walletItem.availableAmount) >= _self.deposits;
        console.log(_self.isBill);
      }
    }, err => {
  ...
  getLstWallet(userId: number) {
    var _self = this;
    _self.commonServices.getWalletByUserId(userId).subscribe(res => {
      if(res.result.success) {
        _self.lstWallet = res.result.data;
        _self.walletItem = _.find(_self.lstWallet, function(o) { return o.isDefault+"" === "1"; });
