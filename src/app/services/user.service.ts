...
import { ApiApplication, paging,Usersetting } from 'app/config/app.config';
...
export class UserService extends ApiService {
        ...
        if (lUserSettings && lUserSettings.length) {
          ...
          if (configNotification) {
            lUserSettings.forEach((setting) => {
              if (setting.settingCode == Usersetting.ShowNotification) {
   ...
   GetListWarehouseVN() {
    ...
    AddNewAffiliate(data: User) {
        return this.post(this.apiBaseController + ApiApplication.user.regAffiliateUser, data);
    }
    ...
    resetPass(mail:any){
        return this.post(this.apiBaseController + ApiApplication.user.resetPass + '?toMailAddress=' + mail);
    }
