...
export class ShipManagerService extends ApiService {
    ...
    cancelOrderConsignment(orderId: number) {
        return this.put(this.apiBaseController + ApiApplication.shipManager.cancelOrderConsignment
                + '?orderId='+orderId);
    }
    ...
    //API danh sách đơn hàng quản lý
    getListOrdersAffiliate(data, page, perPage) {
        return this.post(this.apiBaseController + ApiApplication.shipManager.searchOrdersAffiliate + `?perPage=${perPage}&page=${page}`,data)
    }
