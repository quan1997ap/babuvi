...
export class ApiService {
    ...
    get(apiUrl: string = '', parameter: any = []) {
        ...
        return this.http.get(apiUrl, this.getOptions()).map(
            (res: Response) => {
                ...
                //nếu lỗi do hết hạn token --> refreshToken mới
                if (Number(error.status) === HttpStatus.UNAUTHORIZED) {
                    this.refreshToken();
                    //gọi lại api đang chạy để thực hiện lại
                    return this.http.get(apiUrl, this.getOptions()).map(
                        (res: Response) => {
                            return this.forwardData(res);
                        }).catch(error => {
                            //nếu vẫn bị lỗi --> return lỗi
        ...
        return this.http.request(new Request(requestoptions)).map(
            (res: Response) => {
                ...
                if (Number(error.status) === HttpStatus.UNAUTHORIZED) {
                    ...
                    //lấy lại thông tin header có token sau khi refresh
                    requestoptions.headers = this.getJsonHeaders();
                    //gọi lại api đang chạy để thực hiện lại
                    return this.http.request(new Request(requestoptions)).map(
                        (res: Response) => {
                            return this.forwardData(res);
    ...
    */
    forwardError(error: Response): any {
        ...
        //  else if(Number(error.status) === HttpStatus.UNAUTHORIZED){
        //     this.refreshToken();
        // }
        else {
    ...
    refreshToken() {
      ...
      if(jsonData) {
        const refesh: RefeshTokenModel = {
          accesstoken: jsonData.accessToken,
          refreshtokencode : jsonData.refreshTokenCode,
          ClientAppCode: 'exo-r9045HKopew8mnGj'
        }
        this.post(ApiApplication.user.controller + ApiApplication.user.refreshToken, refesh).subscribe(res => {
