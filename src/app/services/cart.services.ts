...
export class CartServices extends ApiService {
    ...
    getAllServicesOption() {
        return this.get(this.apiBaseController +  ApiApplication.cart.lstAllServicesOption )
        .map(rs => {
    ...
    getAllItemsListCart(userId: number, perPage: number, page: number) {
        return this.get(this.apiBaseController +  ApiApplication.cart.lstCart + '?'
    ...
    callFeeServicesInCart(lServiceId: number, lCartId: number) {
        return this.post(this.apiBaseController +  ApiApplication.cart.addFeeServices + '?'
    ...
    delFeeServicesInCart(lCartServiceId: number) {
        return this.deleteWithParams(this.apiBaseController +  ApiApplication.cart.delPriceServices, lCartServiceId);
    }
    
    // api/Cart/deleteShopItem/11  // xóa shop items
    delShopItem(lCartItemId: number) {
        return this.deleteWithParams(this.apiBaseController +  ApiApplication.cart.deleteShopItem, lCartItemId);
    }

    // xóa shop
    //api/Cart/deleteShop/11
    delShop(lCartId: number) {
        return this.deleteWithParams(this.apiBaseController +  ApiApplication.cart.deleteShop, lCartId);
    }

    // api/Cart/getListDeliveryAddressByUserId?userId=1
    getAllAddress(userId: number) {
        return this.get(this.apiBaseController +  ApiApplication.cart.getListDeliveryAddressByUserId + '?'
                        + ApiApplication.userId + '=' + userId);
    }

    // thêm địa chỉ nhận hàng
    addNewAddress(address: NewAddress){
        return this.post(this.apiBaseController +  ApiApplication.cart.addDeliveryAddress, address);
    }

    // thêm địa chỉ nhận hàng
    updateAddress(address: NewAddress){
        return this.post(this.apiBaseController +  ApiApplication.cart.updateDeliveryAddress, address);
    }
    
    // xóa địa chỉ nhận hàng
    delAddress(addressId: number) {
        return this.put(this.apiBaseController +  ApiApplication.cart.deactiveDeliveryAddressById + '?deliveryAddressId=' + addressId);
    }
    ...
    checkDepositListOrder(deposit: BookingProductModel) {
        return this.put(this.apiBaseController +  ApiApplication.cart.depositLsProduct, deposit);
    }

    updateNoteCart(cart: AllLstCart) {
        return this.put(this.apiBaseController +  ApiApplication.cart.UpdateNoteCart, cart);
    }

    updateNoteItem(item: ItemsOnShop) {
        return this.put(this.apiBaseController +  ApiApplication.cart.UpdateNoteItem, item);
    }

    updateQuanlityItem(item: ItemsOnShop) {
        return this.put(this.apiBaseController +  ApiApplication.cart.UpdateQuanlity, item);
    }
    ...
    getLsServiceConsignment() {
        // danh sách dịch vụ
        return this.get(this.apiBaseController + 'getLsServiceConsignment').pipe(map((res: any) => {
    ...
    getListClassification() {
        ...
        return this.get(this.apiBaseController + 'getListClassification').pipe(map((res: any) => {
            if (res.message === 'successful') {
                // success -->
                return res.result.data;
            } else {
    ...
    getListDeliveryAddressByUserId(userId) {
        ...
        return this.get(this.apiBaseController + 'getListDeliveryAddressByUserId/?userId=' + userId).pipe(map((res: any) => {
            if (res.message === 'successful') {
                // success -->
                return res.result.data;
            } else {
    ...
    addDeliveryAddress(params) {
        ...
        return this.post(this.apiBaseController + 'addDeliveryAddress', params).pipe(map((res: any) => {
            if (res.message === 'successful') {
                // success -->
                return res.result.data;
            } else {
    ...
    deactiveDeliveryAddressById(deliveryAddressId) {
        // lưu địa chỉ nhận hàng
        const url = this.apiBaseController + 'deactiveDeliveryAddressById/?deliveryAddressId' + deliveryAddressId;
