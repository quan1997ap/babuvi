...
@Component({
    selector: 'app-order-buy-rework',
    templateUrl: './order-buy-rework.component.html',
    styleUrls: ['./order-buy-rework.component.scss']
})
export class OrderBuyReworkComponent implements OnInit {
     ...
     */
     saveOrderForm() {
         ...
         this.loading = true;
         this.orderService.orderBuyRework(this.orderBuy).toPromise()
             .then((res) => {
                 this.loading = false;
                 if (res.result.success) {
                     this.getOrderBuy(this.orderId);
