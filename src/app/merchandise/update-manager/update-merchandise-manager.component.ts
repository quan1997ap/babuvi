...
@Component({
    ...
    templateUrl: './update-merchandise-manager.component.html',
    styleUrls: ['./update-merchandise-manager.component.scss']
})
export class UpdateMerchandiseManagerComponent implements OnInit {
    ...
    saveMerchandise(merchandiseForm: NgForm, event) {
        if (merchandiseForm.valid) {
            this.loading = true;
            this.merchandiseServices.editMerchandiseManager(this.merchandises).toPromise()
                .then((res) => {
                    this.loading = false;
                    if (res.result.success) {
                        this.loadMerchandiseList(this.merchandises.orderCode, this.page.size, this.page.pageNumber + 1);
