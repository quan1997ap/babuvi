...
@Component({
  selector: 'app-orders-affiliate',
  templateUrl: './orders-affiliate.component.html',
  styleUrls: ['./orders-affiliate.component.scss']
})
export class OrdersAffiliateComponent implements OnInit {
  ...
  getDataFromServer() {
    this._passData.loading(true);
    this._shipManager.getListOrdersAffiliate(this.nullForm, 1, 5).subscribe(res => {
  ...
  async loadDataFromServerPerPage(numb) {
    this._passData.loading(true);
    await this._shipManager.getListOrdersStaff(this.nullForm, numb, 5).subscribe( async res => {
      if (res.result.success) {
        this.dataItem = res.result.data;
        // page
        this.dataCount = this.dataItem.dataCount;
