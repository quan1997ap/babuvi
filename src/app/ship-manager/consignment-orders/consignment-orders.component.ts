...
@Component({
   ...
   templateUrl: './consignment-orders.component.html',
   styleUrls: ['./consignment-orders.component.scss']
})
export class ConsignmentOrdersComponent implements OnInit {
   ...
   getDataFromServer() {
      this._passData.loading(true);
      this._shipManager.getConsignmentOrders(this.nullForm, 1, 5).subscribe(res => {
         console.log(res)
   ...
   filter(status) {console.log(status)
 ...
 async loadDataFromServerPerPage(numb) {console.log(numb)
   this._passData.loading(true);
   await this._shipManager.getConsignmentOrders(this.nullForm, numb, 5).subscribe( async res => {
     console.log(res)
     if (res.result.success) {
       this.dataItem = res.result.data;
       // page
